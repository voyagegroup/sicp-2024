...
(define (estimate-integral P x1 x2 y1 y2)
  ...
  (define (test passed failed)
    (let* ((in? (P (random-in-range x1 x2) (random-in-range y1 y2)))
           (new-passed (if in? (+ passed 1) passed))
           (new-failed (if in? failed (+ failed 1))))
      (cons-stream (* (/ new-passed (+ new-passed new-failed)) rect-area)
                   (test new-passed new-failed))))
  (test 0 0))

(define (estimate-pi n)
  (define (in-unit-circle? x y)
    (<= (+ (square x) (square y)) 1))
  (stream-ref (estimate-integral in-unit-circle? -1.0 1.0 -1.0 1.0) n))
