...
(define (make-zero-crossings input-stream last-value last-avg)
  (if (stream-null? input-stream)
      the-empty-stream
      (let ((avpt (/ (+ (stream-car input-stream) last-value) 2)))
        (cons-stream (sign-change-detector avpt last-avg)  ; 修正（last-value → last-avg）。もともとは平滑化した値と生データとをひかくしていた。平滑化した値同士を比較するように修正。
                     (make-zero-crossings (stream-cdr input-stream)
                                          (stream-car input-stream)
                                          avpt)))))


(display-stream-n (make-zero-crossings sense-data 0 0) 13)
