...
(define (sign-change-detector n1 n2)
  (cond ((and (>= n1 0) (< n2 0)) 1)
        ((and (< n1 0) (>= n2 0)) -1)
        (else 0)))

(define sense-data (cons-stream 1 (cons-stream 2 (cons-stream -1 ones))))
...
; 元の版は、平滑化したavptをlast-valueにしていたため、直前の２値の平均ではなくそれ以前の値の影響も残ってしまっていた。
; last-valueを直前の値にし、last-avptで平滑化した値を保持するように変更した。
; また、比較もavpt同士で行うように修正した。
