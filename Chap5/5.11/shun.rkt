...
(controller
 ...
 afterfib-n-2                         ; 戻った時Fib(n-2)の値はvalにある
   ...
   (assign val                        ; Fib(n-2)+Fib(n-1)
           (op +) (reg val) (reg n))  ; valにはFib(n-2)があるので同じ意味になる
   (goto (reg continue))              
 immediate-answer
   (assign val (reg n))               
   (goto (reg continue))
    ...
    (lambda ()
      (let ((saved (pop stack)))
        (if (eq? reg-name (car saved))
            (set-contents! reg (cadr saved))
            (error "Wrong register" reg-name)))
      (advance-pc pc))))


; c

; registerにstackを持つようにし、各レジスタのstackを初期化するように修正しあ
...
(define (make-register name)
  (let ((contents '*unassinged*)
        (stack '())) ; 
    (define (save)
      (set! stack (cons contents stack)))
    (define (restore)
      (if (null? stack)
          (error "empty stack")
          (begin
            (set! contents (car stack))
            (set! stack (cdr stack)))))
      ...
      (define (initialize-stacks)
        (for-each
         (lambda (register-entry)
           ((cadr register-entry) 'initialize-stack))
         register-table))
      (let ((the-ops
