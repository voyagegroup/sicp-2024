...
;; 5.11 b
;; saveするとき ((y 20) (x 10)) のような形式にして、restoreすつときにkeyを確認する
(define (make-save inst machine stack pc)
  (let ((reg-name (stack-inst-reg-name inst)))
    (let ((reg (get-register machine reg-name)))
      (lambda ()
        (push stack (list reg-name (get-contents reg))) ; (cons reg-name content) へ
        (advance-pc pc)))))
...
(define (make-restore inst machine stack pc)
  (let ((reg-name (stack-inst-reg-name inst)))
    (let ((reg (get-register machine reg-name)))
      (lambda ()
        (let ((top (pop stack)))
          (if (eq? (car top) reg-name) ; nameをチェック
              (set-contents! reg (cadr top))
              (error "wrong register name" reg-name)))
        (advance-pc pc)))))
