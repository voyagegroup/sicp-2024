...
(define (make-stack)
  (let ((s '()) (total-pushes 0) (current-depth 0) (maximum-depth 0))
    (define (push x)
      (set! s (cons x s))
      (set! total-pushes (+ total-pushes 1))
      (set! current-depth (+ current-depth 1))
      (if (> current-depth maximum-depth)
          (set! maximum-depth current-depth)))
    (define (pop)
      (if (null? s)
    ...
    (define (print-statistics)
      (display (list 'total-pushes '= total-pushes
...
; 2 x (n - 1)で退避演算の全数とスタックの最大深さnが求められる