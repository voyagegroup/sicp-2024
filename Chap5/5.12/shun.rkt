...
(define (make-register name)
  (let ((contents '*unassinged*))
    (define (dispatch message)
      (cond ((eq? message 'get) contents)
            ((eq? message 'set)
             (lambda (value) (set! contents value)))
            (else
             (error "Unknown request -- REGSTER" message))))
    dispatch))
    ...
    (let ((the-ops
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'instruction-list) instructions)
    ...
    (for-each
     (lambda (inst)
       (let ((text (instruction-text inst)))
         ((machine 'add-instruction) text) 
         (cond
           ((eq? (car text) 'goto) 
            (let ((dest (goto-dest text)))
              (if (register-exp? dest)
                  ((machine 'add-goto-register) (register-exp-reg dest)))))
           ((or (eq? (car text) 'save) (eq? (car text) 'restore)) ((machine 'add-stack-register) (cadr text)))
           ((eq? (car text) 'assign) ((machine 'add-register-source) (assign-reg-name text) (assign-value-exp text))))
         )
