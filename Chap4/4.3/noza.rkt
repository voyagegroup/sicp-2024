#lang sicp
; 方針: データ主導のディスパッチ表を導入し、式型ごとに手続きを登録。
...
(define (put op proc)
  (set! eval-dispatch-table (cons (cons op proc) eval-dispatch-table)))

(define (get op)
  (let ((record (assoc op eval-dispatch-table)))
...
(define (eval exp env)
  (cond
    ...
    (else
     (let* ((type (if (pair? exp) (car exp) 'unknown))
            (proc (or (get type)
                      (and (pair? exp) (get 'application)))))
       (if proc
           (proc exp env)
