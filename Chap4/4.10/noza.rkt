#lang sicp
; 方針: eval/apply を変えず、新しい構文トークンに対応する抽象構文を用意。
...
; begin   -> (do <exp1> <exp2> ...)
; cond    -> (case (<test> <action...>) ... (else <action...>))
; call    -> (call <proc> <arg> ...)

; クォート式は (q <text-of-quotation>) の形である。
(define (quoted? exp)
  (tagged-list? exp 'q))
...
; 定義は (def <var> <value>) または (def (<var> <parameter1> ... <parameterN> <body>) の形である
...
; 手続き作用は (call <operator> <operand> ...) の形
...
(define (test-4.10)
  (eval '(do
           (def lst (call cons (q a) (call cons (q b) (q ()))))
           (def rev
             (fn (xs acc)
               (if* (call null? xs)
                    acc
