...
(define (make-frame variables values)
  (define (itr vars vals acm)
    (if (or (null? vars) (null? vals))
        acm
        (itr (cdr vars) (cdr vals) (cons (cons (car vars) (car vals)) acm))))
  (itr variables values '()))

(define (frame-variables frame) (map car frame))

(define (frame-values frame) (map cdr frame))

; 既存のframeをcdrに入れてframeを更新する
...
(define (delete-by-car! key lst-cell)
  (let loop ((prev lst-cell))
    (let ((cur (cdr prev)))
      (cond
        ((null? cur) 'done)
        ((equal? (caar cur) key)
         (set-cdr! prev (cdr cur))
         (loop prev))  ; 同じprevから続ける(連続削除のため)
        (else
         (loop cur))))))
...
(define (eval-unbind! exp env)
  (let ((frame (first-frame env))
    ...
    (cond
      ((null? frame) 'done)
      ; 先頭が一致する場合はdelete-by-car!の外で処理する(先頭はset-car!とset-cdr!で更新するため)
      ((equal? (caar frame) var)
       (set-car! frame (cadr frame))
       (set-cdr! frame (cddr frame)))
      (else
       (delete-by-car! var frame)))))
...
; グローバルなものまで消せてしまうとtrueやfalseも消えてしまうため、最初のフレームだけを削除するようにした。