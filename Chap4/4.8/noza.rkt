...
(define (named-let->combination exp)
  (let* ((name (let-name exp))
         ...
         (vals (let-values exp))
         (body (let-body exp))
         (define-form (cons 'define (cons (cons name vars) body)))
         (call-form (cons name vals)))
    (list (make-lambda '()
                       (list define-form call-form)))))
