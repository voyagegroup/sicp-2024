...
; 特殊形式としての実装は高階手続に渡すことができない。
; なぜなら、構文の変更ルールであり、変数のように定義されているものではない。
; `(map unless '(#t #f) '(1  2) '(10 20))`みたいなことをしようとしてみても、
; 'unlessはanalyzeのvariable?に引っかかるがunlessというシンボルはどこにも定義されていないのでエラーになる。
; 手続であれば`(10 2)`を返すことができる。