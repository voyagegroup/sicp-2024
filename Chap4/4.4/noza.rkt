#lang sicp
; 方針: and/or を特殊形式として実装し、導出式への変換も用意。
...
(define (eval-and exp env)
  (define (iter clauses)
    (cond ((null? clauses) true)
          ...
          (else
           (let ((value (eval (car clauses) env)))
             (if (false? value)
                 false
                 (iter (cdr clauses)))))))
...
(define (or->if exp)
  (define (expand clauses)
    (cond ((null? clauses) false)
          ((null? (cdr clauses)) (car clauses))
          (else
           (let ((value-var 'or-value))
             (list (make-lambda (list value-var)
                                (list (make-if value-var
                                               value-var
                                               (expand (cdr clauses)))))
                   (car clauses))))))
...
; 問題4.4 確認用
(define (test-4.4)
  (list (eval '(and true 1 2) the-global-environment)
        (eval '(and false 1) the-global-environment)
        (eval '(or false 7) the-global-environment)
