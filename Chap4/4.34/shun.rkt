...
(define (list->cons l)
  (if (null? l)
      ''()
      (list 'cons
            (list 'quote (car l)) ; 変数ではなくシンボルとして扱う
            (list->cons (cdr l)))))
...
;(procedure (m) ((m (quote lazy-pair) x y)) <env>)
(define (lazy-pair? obj)
  (and (compound-procedure? obj)
       (let ((body (procedure-body obj)))
         (and (pair? body)
              (let ((exp (car body)))
                (and (pair? exp)
                     (pair? (cdr exp))
                     (quoted? (cadr exp))
                     (eq? (text-of-quotation (cadr exp)) 'lazy-pair)))))))

(define (print-lazy-list obj limit)
  (display "(")
  (print-lazy-list-elements obj limit)
  (display ")"))

(define (print-lazy-list-elements obj limit)
  (cond ((= limit 0)
         (display "..."))
        ((null? obj)
         'done)
        ((lazy-pair? obj)
         (user-print (lazy-pair-car obj))
         (let ((rest (lazy-pair-cdr obj)))
           (cond ((null? rest)
                  'done)
                 ((lazy-pair? rest)
                  (display " ")
                  (print-lazy-list-elements rest (- limit 1)))
                 (else
                  (display " . ")
                  (user-print rest)))))
        (else
...
(define (lazy-pair-exp obj)
  (car (procedure-body obj)))

(define (lazy-pair-car obj)
  (let ((exp (lazy-pair-exp obj)))
    (force-it
     (lookup-variable-value (caddr exp)
                            (procedure-environment obj)))))

(define (lazy-pair-cdr obj)
  (let ((exp (lazy-pair-exp obj)))
    (force-it
     (lookup-variable-value (cadddr exp)
                            (procedure-environment obj)))))
