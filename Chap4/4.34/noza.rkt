...
(define (eval exp env)
  (cond
    ...
    ; letrec は let + set! へ変換して評価する(問題 4.20a)
    ((letrec? exp) (eval (letrec->let exp) env))
    ; let は導出された式として扱う
    ((let? exp) (eval (let->combination exp) env))
    ((application? exp)
     (my-apply (actual-value (operator exp) env)
               (operands exp) ; ここで引数を評価しなくなった
               env))
    ; 対応できない場合
    (else
  ...
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           (procedure-parameters procedure)
           (list-of-delayed-memo-args arguments env)
           (procedure-environment procedure))))
        (else
         (error
          "Unknown procedure type -- APPLY" procedure))))
...
(define (letrec-values exp)
  (map binding-value (letrec-bindings exp)))
...
(define (scan-out-defines body)
  ; partition-definitions: body を2つに分割
  ; - definitions: define 式だけ
  ; - others: それ以外の式
  (define (partition-definitions exps defs others)
    (if (null? exps)
        (cons (reverse defs) (reverse others))
        (let ((exp (car exps)))
          (if (definition? exp)
              (partition-definitions (cdr exps) (cons exp defs) others)
              (partition-definitions (cdr exps) defs (cons exp others))))))
  (let* ((parts (partition-definitions body '() '()))
         (definitions (car parts))
         (remaining-body (cdr parts)))
    (if (null? definitions)
        body
        (list
         (make-let
...
(define (make-procedure parameters body env)
  ; c. make-procedure で一度だけ scan するほうが、procedure-body のたびに
  ;    変換を繰り返さず効率がよい。
...
; 問題 4.34: cons の dispatch パラメータを lazy-pair に変更してメタレベルから識別可能に。
(eval '(define (cons x y) (lambda (lazy-pair) (lazy-pair x y))) the-global-environment)
...
(define (user-print object)
  (cond ((lazy-pair? object)
         (display "(")
         (user-print-lazy-pair object 10) ; lazy-pair の表示は別扱い
         (display ")"))
...
; 4.34. 向けに作成
; limit までを表示するように
(define (user-print-lazy-pair pair limit)
  (let ((a (lazy-pair-car pair)))
    (user-print a)
    (if (= limit 0)
        (display " ...")
        (let ((d (lazy-pair-cdr pair)))
          (cond ((null? d) 'done)
                ((lazy-pair? d)
                 (display " ")
                 (user-print-lazy-pair d (- limit 1)))
                (else
                 (display " . ")
                 (user-print d)))))))
...
(define (list-of-delayed-memo-args exps env)
  (if (no-operands? exps)
      '()
      (cons (delay-memo-it (first-operand exps) env)
            (list-of-delayed-memo-args (rest-operands exps) env))))
...
; 問題 4.34: パラメータ名 lazy-pair を目印に遅延対を識別する
