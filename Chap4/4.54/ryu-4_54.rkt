...
; (choose-nth '(a b c d) 2)
; -> (c a b d)
(define (choose-nth xs n)
  (define (iter before rest n)
    (cond ((null? rest)
           (error "index out of range -- CHOOSE-NTH" n))
          ((= n 0)
           (cons (car rest)
                 (append (reverse before) (cdr rest))))
          (else
           (iter (cons (car rest) before)
                 (cdr rest)
                 (- n 1)))))
  (iter '() xs n))
...
(define (shuffle xs)
  (if (null? xs)
      '()
      (let ((picked (choose-nth xs (random (length xs)))))
        (cons (car picked)
              (shuffle (cdr picked))))))
        ...
        (if (null? choices)
            ...
            ((car choices) env
                           ...
                           (lambda ()
                             (try-next (cdr choices))))))
      ; (try-next cprocs))))
      (try-next (shuffle cprocs))))) ; shuffleするようにする

; --- 4.51 permanet-set!の実装
...
(define (if-fail-consenquent exp) (cadr exp))
...
(define (analyze-if-fail exp)
  (let ((cproc (analyze (if-fail-consenquent exp))) ; true
        (aproc (analyze (if-fail-alternative exp)))) ; else
    (lambda (env succeed fail)
      (cproc env
             succeed ; 成功は通常どおり
             (lambda () ; 失敗は継続
