...
(define (my-apply procedure arguments env)
  (cond ((primitive-procedure? procedure)
         (apply-primitive-procedure
          procedure
          (list-of-arg-values arguments env)))  ; 4.2.2 変更した
        ((compound-procedure? procedure)
         (eval-sequence
          (procedure-body procedure)
          (extend-environment
           (procedure-parameters procedure)
           (list-of-delayed-args arguments env) ; 4.2.2 変更した
...
(define (text-of-quotation exp env)
  (if (list? (cadr exp)) ; '(a b c) のようなクォートのリストかどうか
      (eval (quote->exp (cadr exp)) env)
      (cadr exp))) ; リストじゃなかったら今まで通り

(define (make-quatetion val)
  (list 'quote val))

(define (make-cons car cdr)
  (list 'cons car cdr))

(define (quote->exp quate-list)
  (if (null? quate-list)
      (make-quatetion '())
      (make-cons
       (make-quatetion (car quate-list))
       (quote->exp (cdr quate-list)))))
...
; --- 4.3
(put 'eval 'quote
     (lambda (exp env) (text-of-quotation exp env)))
