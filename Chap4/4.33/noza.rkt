         ...
         (let ((parameters (procedure-parameters procedure)))
           (eval-sequence
            ...
            (extend-environment
             ...
             (list-of-parameter-args arguments
                                     parameters
                                     env)
             (procedure-environment procedure)))))
        (else
...
(define (parameter-mode-symbol? mode)
  (or (eq? mode 'lazy)
      (eq? mode 'lazy-memo)))

(define (parameter-declaration? parameter)
  (and (pair? parameter)
       (pair? (cdr parameter))
       (null? (cddr parameter))
       (symbol? (car parameter))
       (parameter-mode-symbol? (cadr parameter))))

(define (parameter-name parameter)
  (cond ((symbol? parameter) parameter)
        ((parameter-declaration? parameter)
         (car parameter))
        (else
         (error "Invalid parameter declaration" parameter))))

(define (parameter-mode parameter)
  (cond ((symbol? parameter) 'strict)
        ((parameter-declaration? parameter)
         (cadr parameter))
        (else
         (error "Invalid parameter declaration" parameter))))
...
(define (list-of-parameter-args exps parameters env)
  (if (or (no-operands? exps)
          (null? parameters))
      '()
      (cons (argument-value-by-mode (first-operand exps)
                                    (parameter-mode (car parameters))
                                    env)
            (list-of-parameter-args (rest-operands exps)
                                    (cdr parameters)
                                    env))))

(define (argument-value-by-mode exp mode env)
  (cond ((eq? mode 'strict)
