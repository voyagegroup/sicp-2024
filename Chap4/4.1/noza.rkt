...
(define (list-of-values exps env)
  (list-of-values-right-to-left exps env))

; 問題 4.1 の挙動確認用
; 左→右なら trace は (c b a)、右→左なら (a b c) になる
(define (test-4.1)
  (define-variable! 'trace '() the-global-environment)
  (list-of-values
   '((begin (set! trace (cons 'a trace)) 'a)
     (begin (set! trace (cons 'b trace)) 'b)
     (begin (set! trace (cons 'c trace)) 'c))
   the-global-environment)
  (lookup-variable-value 'trace the-global-environment))
