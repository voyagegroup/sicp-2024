...
;
;;;; L-Eval value:
;2 →wを実行するとcountはさらに１増えている。


; 答えの理由
; actual-valueが呼ばれるタイミングでcountは増加する。
; wを定義した時、eval-definitionは引数(id (id 10))に対してevalを呼び出す。
; これがmy-applyを呼び出され、手続き本体はeval-sequenceによって実行されるためcountが増加する。
; ただし、合成手続きであるため、内側の(id 10)は遅延されたサンクになり、それが返却される。
; wを実行すると、(id 10)のサンクが呼び出されてforce-itされてcountが増加する。