         ...
         (loop-def
          (cons 'define
                (cons (list loop-name)
                      (list (make-if test
                                     (sequence->exp
                                      (append body (list (list loop-name))))
                                     'false)))))
         (call-loop (list loop-name)))
    (list (make-lambda '() (list loop-def call-loop)))))
...
(define (test-4.9)
  (eval '(begin
           ...
           (define acc '())
           (while (if (null? x) false true)
             (set! acc (cons (car x) acc))
             (set! x (cdr x)))
           acc)
