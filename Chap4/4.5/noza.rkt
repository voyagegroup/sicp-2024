#lang sicp
; 方針: cond の (test => recipient) 形式を変換で処理。
...
(define (cond-recipient-clause? clause)
  (and (pair? (cond-actions clause))
       ...
       (pair? (cdr (cond-actions clause)))
       (null? (cddr (cond-actions clause)))))
      ...
      (let ((first (car clauses))
        ...
        (if (cond-else-clause? first)
            ...
            (if (cond-recipient-clause? first)
                (let ((value-var 'cond-value))
                  (list (make-lambda (list value-var)
                                     (list (make-if value-var
                                                    (list (cond-recipient first) value-var)
                                                    (expand-clauses rest))))
...
; 問題4.5 確認用
(define (test-4.5)
  (eval '(cond ((if true 7 false) => (lambda (x) (cons x '())))
               (else false))
