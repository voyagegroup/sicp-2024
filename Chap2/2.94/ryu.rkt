...
(define (install-scheme-number-package)
  ...
  (put 'gcd '(scheme-number scheme-number)
       (lambda (x y) (make-scheme-number (gcd x y))))
  'done)
...
;; 2.94のテスト用多項式作成を修正
(define p2-1 (make-polynomial 'x 
                              (list (make-term 4 (make-scheme-number 1))
                                    (make-term 3 (make-scheme-number -1))
                                    (make-term 2 (make-scheme-number -2))
                                    (make-term 1 (make-scheme-number 2)))))
(define p2-2 (make-polynomial 'x 
                              (list (make-term 3 (make-scheme-number 1))
                                    (make-term 1 (make-scheme-number -1)))))
(greatest-common-divisor p2-1 p2-2)
