    ...
    (let ((proc (get op type-tags)))
      (if proc
          (let ((result (apply proc (map contents args))))
            ...
            (if (memq op '(project drop raise equ? real-part imag-part magnitude angle))
                result
                (drop result)))
            ;; ---------------------------------------------------------------
          (if (= (length args) 2)
...
(define (can-drop? x)
  (let ((type (type-tag x)))
    (cond [(eq? type 'scheme-number) #f]  ; scheme-numberは最下層
          ...
          [(eq? type 'complex)
           ;; complexの虚部が0かチェック（タグ付きデータ対応）
           (let ((z-contents (contents x)))
             (if (and (pair? z-contents) (eq? (car z-contents) 'rectangular))
                 (let* ((rect-data (cdr z-contents))
                        (imag-tagged (cdr rect-data)))
                   (cond [(and (pair? imag-tagged) (eq? (car imag-tagged) 'scheme-number))
                          (= (cdr imag-tagged) 0)]
                         [(and (pair? imag-tagged) (eq? (car imag-tagged) 'rational))
                          (= (cadr imag-tagged) 0)]  ; 分子が0
                         [else #f]))
                 #f))]
          [else #f])))
#| ------------------------------------------------------------------------------|#
...
(provide add sub mul div exp raise equ? project drop
         sine cosine square-root arctan square
