...
(define scheme-number1  ((get 'make 'scheme-number) 5))

(define scheme-number2
  ((get 'make 'scheme-number) 6))

(add scheme-number1 scheme-number2)
(exp scheme-number1 scheme-number2)

(define complex1
  ((get 'make-from-real-imag 'complex) 3 4))

(define complex2
  ((get 'make-from-real-imag 'complex) 4 5))

(add complex1 complex2)
; (exp complex1 complex2) ; エラーになるためコメントアウト
(exp complex1 complex2)

;; 定義がされいるものについてはついては正しく処理される
;; 定義がされていないものについては、処理が終わらない
;; 手続きを探して対応するものがなければ型変換を行おうとする。
;; 自身に型変換するためまた同じ型で proc がないかを探し、ないのでまた型変換をしようとする
;; が続くことになるので無限ループになる

;; b
;; 正しくない
;; a のように手続きが存在しなかった場合に処理が終わらなくなってしまう。
;;
;; apply-generic は正しく動作する
